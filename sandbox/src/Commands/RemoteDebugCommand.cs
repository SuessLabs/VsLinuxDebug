...
namespace VSRemoteDebugger
{
    ...
    internal sealed class RemoteDebugCommand
    {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            if (Settings.NoDebug)
                            {
                                Mbox("Files successfully transfered to remote machine", "Success");
                            }
                            else
                            {
        ...
        private async Task<string> TransferFiles2Async()
        {
            ...
            try
            {
                    ...
                    : null) //// new SftpClient(Settings.IP, Settings.HostPort, Settings.UserName, Settings.SshKeyFile))
                {
                    ...
                    PayloadCompressAndUpload(sftp, srcDirInfo, pathTarGz);
        ...
        ////                Arguments = $@"-r {_localhost.OutputDirFullName}\* {Settings.UserName}@{Settings.IP}:{Settings.DebugFolderPath}",
        ...
        {
            // Get local files to transfer
            var localFiles = GetLocalFiles(srcDirInfo);

            // TODO: Add try/catch and return false on error
